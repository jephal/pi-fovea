...
export const classifyLiteral = (text: string): LitClass | undefined => {
  ...
  if (WORD_RE.test(t) && (t.includes(".") || t.includes("::") || t.includes("-") || /[A-Z][a-z]/.test(t.slice(1)))) {
    return "word";
  }
  return undefined;
};
...
export const buildJoinIndex = (
  ...
  const grouped = new Map<string, { cls: LitClass; occ: LitOccurrence[] }>();
  let total = 0;
  for (const s of sites) {
    ...
    const g = grouped.get(key) ?? { cls, occ: [] };
    grouped.set(key, g);
  ...
  for (const [key, g] of grouped) {
    ...
    const spec = Math.min(1, Math.log(total / df) / idfMax || 0);
    const w = BASE[g.cls] * (0.25 + 0.75 * spec);
    byKey.set(key, { cls: g.cls, spec, occ: g.occ });
